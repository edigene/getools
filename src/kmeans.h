/* reconstructed from DKM
 * the original source code can be found in https://github.com/genbattle/dkm
 * I have changed some default behavior and add some functionality for better use for this project
 */
#ifndef KMEANS_H
#define KMEANS_H
...
namespace details {
    ...
    // initialization method based on the [kmeans++](https://en.wikipedia.org/wiki/K-means%2B%2B)
    template <typename T, size_t N>
    ...
    std::vector<uint32_t> calculate_clusters(const std::vector<std::array<T, N>>& data, const std::vector<std::array<T, N>>& means){
        ...
        for(auto& point : data) clusters.push_back(closest_mean(point, means));
        return clusters;
    }
   ...
   std::vector<std::array<T, N>> calculate_means(const std::vector<std::array<T, N>>& data, const std::vector<uint32_t>& clusters, const std::vector<std::array<T, N>>& old_means, uint32_t k){
       ...
       for(size_t i = 0; i < k; ++i){
           if(count[i] == 0) means[i] = old_means[i];
               ...
               for(size_t j = 0; j < means[i].size(); ++j) means[i][j] /= count[i];
           }
       }
       return means;
   }
   ...
   std::vector<T> deltas(const std::vector<std::array<T, N>>& old_means, const std::vector<std::array<T, N>>& means){
       ...
       for(size_t i = 0; i < means.size(); ++i) distances.push_back(distance(means[i], old_means[i]));
       return distances;
   }
   
   template <typename T>
   bool deltas_below_limit(const std::vector<T>& deltas, T min_delta){
       for(T d : deltas) if(d > min_delta) return false;
       return true;
   }
} // namespace details
...
/*
ClusterParameters is the configuration used for running the kmeans_lloyd algorithm.
...
class ClusterParameters{
    public:
        explicit ClusterParameters(uint32_t k){
            _k = k;
            _has_max_iter = false;
            _max_iter = 0;
...
/*
Implementation of k-means generic along the data type and the dimension of each data item. Expects
...
Takes a `ClusterParameters` struct for algorithm configuration. See the comments for the
`ClusterParameters` struct for more information about the configuration values and how they
...
template <typename T, size_t N>
std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>> kmeans_lloyd(const std::vector<std::array<T, N>>& data, const ClusterParameters<T>& parameters){
...
`ClusterParameters` struct for configuration.
*/
template <typename T, size_t N>
std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>> kmeans_lloyd(const std::vector<std::array<T, N>>& data, uint32_t k, uint64_t max_iter = 0, T min_delta = -1.0){
    ClusterParameters<T> parameters(k);
    if(max_iter != 0) parameters.set_max_iteration(max_iter);
    if(min_delta != 0) parameters.set_min_delta(min_delta);
    parameters.set_random_seed(47);
   return kmeans_lloyd(data, parameters);
}

// compute the silhouette coefficient of a kmeans result, https://en.wikipedia.org/wiki/Silhouette_(clustering)
template <typename T, size_t N>
double silhouette_coef(std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>>& kmr, const std::vector<std::array<T, N>>& data){
    // get kmeans result
    auto& means = std::get<0>(kmr);
    auto& label = std::get<1>(kmr);
    int k = means.size();
    // count cluster size 
    std::vector<size_t> ksize(k, 0);
    for(auto& l: label) ++ksize[l];
    // init
    std::vector<double> a, b, s;
    a.resize(data.size(), 0);
    b.resize(data.size(), 0);
    ...
    for(size_t i = 0; i < data.size(); ++i){
        ...
        if(j == 1) a[i] = 0;
            ...
            for(size_t z = 0; z < data.size(); ++z){
                if(z != i && label[z] == l){
                    dist += details::distance(data[i], data[z]);
                }
            }
            a[i] = dist/(j-1);
        }
    ...
    for(size_t i = 0; i < data.size(); ++i){
        ...
        for(; j < k; ++j){
              if(j != l){
                  ...
                  dist /= ksize[j];
                  b[i] = std::min(dist, b[i]);
              }
          }
    }
    // calculate s
    for(size_t i = 0 ; i < data.size(); ++i){
        if(ksize[label[i]] == 1) s[i] = 0;
        else s[i] = (b[i] - a[i])/std::max(a[i], b[i]);
    }
    // calculate coef
    double sum = 0;
