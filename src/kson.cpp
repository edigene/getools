...
void kson_usage(){
    fprintf(stderr, "\n");
    fprintf(stderr, "Usage: kson <in.json> [key [...]]\n");
    fprintf(stderr, "\n");
...
int kson_main(int argc, char** argv){
    if(argc < 2){
        kson_usage();
        return 0;
    }
    ...
    if(kson){
        if(argc > 2){
            ...
            if(p){
                ...
                fprintf(stderr, "Failed to find the slot\n");
                kson_destroy(kson);
                return 1;
            }
        }else{
        ...
        fprintf(stderr, "failure parsing input json\n");
        return 1;
    }
}
